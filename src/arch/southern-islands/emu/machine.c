...
int product_change_cnt = 0;
int product_inst_cnt = 0; 
...
void printFloatBitsAdd(float x){ 
     unsigned int *iP = (unsigned int *)&x;
     printBitsAdd(*iP);
     fprintf(infile_FLTADD, "\n");
}

/* xun jiao: 11/09 define dump bits function for sqrt   */
void printBitsSqrt(unsigned int a){ 
   static int flag = 0;  
   if(flag != 32){
      ++flag;
      printBitsSqrt(a/2);
     // printf("%d ", a%2);
      fprintf(infile_SQRT, "%d", a%2); 
      --flag;
//      if(flag == 31 || flag == 23) 
          //putchar(' '); 
   }   
...
void printFloatBitsSqrt(float x){ 
     unsigned int *iP = (unsigned int *)&x;
     printBitsSqrt(*iP);
     fprintf(infile_SQRT, "\n");
}
...
//xun jiao 11/06/14: f2int, random(), swapBits 
...
//05/06/16: this function never use *prob, *bit and width as input, used to swap a bit of sum  
//Input: bit_swap: specify which bit position needs to be swapped 
//Input: r2() > ? the question mark is the probability, (reliability specification) we want
//Input: float x: the sum we want to swap. we use a struct to convert it to its binary format
//Output: the swapped sum 

/* Xun Jiao 01/18/2017 swap the bits for integer number */
int swapBits_int(int orig_int, int bit_swap, float rel_spec)
{
    int new_int; 
    if(r2() > rel_spec)
       new_int = orig_int ^ (1 << (bit_swap)); 
    return new_int; 
}

/* Xun Jiao 01/18/2017 swap the bits for float number */ 
float swapBits_float(float orig_float, int bit_swap, float rel_spec)
{
    f2int f2int_x; 
    f2int_x.input = orig_float; 
    if(r2() > rel_spec)
        f2int_x.output = f2int_x.output ^ (1 << (bit_swap));
    return f2int_x.input; 
}

/* Xun Jiao 01/18/2017 swap the bits for double number */
double swapBits_double(double orig_double, int bit_swap, float rel_spec)
{
    f2int f2int_x; 
    f2int_x.input_double = orig_double; 
    if (r2() > rel_spec) 
        f2int_x.output = f2int_x.output ^ (1 << (bit_swap));
    return f2int_x.input_double;     
}

/* Xun Jiao 01/19/2017 swap the bits for INT_ADD */
int swapBits_INT_ADD(int x)
{
    int bit_swap = 0; 
    float rel_spec = 0.5; 
    if(r2() > rel_spec)
...
/* Xun Jiao 01/19/2017 swap the bits for FP_ADD */
float swapBits_FP_ADD(float x)
{
    int i;
    f2int f2int_a; 
    f2int_a.input = x; 
    ...
    if(r2() > rel_spec)
    {
        f2int_a.output = f2int_a.output ^ (1 << (bit_swap));
    } 
    return f2int_a.input; 
}

/* Xun Jiao 01/19/2017 swap the bits for INT_MUL*/
int swapBits_INT_MUL(int x)
{
    int bit_swap = 31; 
    float rel_spec = 1;  //if set 1, then no bit flip at all 
    if(r2() > rel_spec)
       x = x ^ (1 << (bit_swap)); 
    return x; 
}

/* Xun Jiao 01/19/2017 swap the bits for FP_MUL */
float swapBits_FP_MUL(float x)
{
    int i;
    f2int f2int_a; 
    f2int_a.input = x; 
    int bit_swap = 31; 
    float rel_spec = 0.99995; 
    if(r2() > rel_spec)
    {
        f2int_a.output = f2int_a.output ^ (1 << (bit_swap));
    } 
...
float swapBits_Sqrt(float x, float *prob, int *bit, int width)
{
    int i;
    f2int f2int_a; 
    f2int_a.input = x; 
   // for(i = 0; i < width; i ++)
    ...
    //for(i = 0; i < 24; i ++)
    {
      ...
      //  if(r2() > prob[i])
       ...
       // if(r2() > 0.9999)
        {
	 //   f2int_a.output = f2int_a.output ^ (1 << bit[i]);
            f2int_a.output = f2int_a.output ^ (1 << (i));
      //      printf("The %dth bit is swapped a the probability of %f\n", bit[i], prob[i]);
        } 
    }
    return f2int_a.input; 
}
	...
	struct si_inst_t *inst)
{
	...
	carry.as_uint = ((unsigned long long) s0.as_uint + 
        ...
        /* Xun Jiao 01/19/2017 swap INT_ADD bits */
        if(si_fault_injection_INT_ADD > 0)
            sum.as_uint = swapBits_INT_ADD(sum.as_uint);  
	...
	struct si_inst_t *inst)
{
	...
	if (INST.ssrc1 == 0xFF)
		...
		s1.as_uint = si_isa_read_sreg(work_item, INST.ssrc1);
  //printf("%d\n", si_data_profile);
  /* xun jiao: 05/06/16, dump the input operand */
  if(si_data_profile_INTADD == 1) {
     infile_INTADD = fopen("input_INTADD.txt", "a+");
     printf("creating INTADD file\n");
     si_data_profile_INTADD ++; 
  }
 
  
  if(si_data_profile_INTADD > 0)
  {
      fprintf(infile_INTADD, "%d\t%d\n", s0.as_int, s1.as_int);  
  }
	...
	ovf.as_uint = (s0.as_int >> 31 != s1.as_int >> 31) ? 0 : 
	  ...
	  /* Xun Jiao 01/19/2017 swap INT_ADD bits */
	  if(si_fault_injection_INT_ADD > 0)
	      sum.as_int = swapBits_INT_ADD(sum.as_int); 
	...
	struct si_inst_t *inst)
{
  ...
  //printf("INT MUL IS EXECUTED");

/* XUN JIAO 05/06/16 dump input operands for MUL */
  if(si_data_profile_INTMUL == 1) {
     infile_INTMUL = fopen("input_INTMUL.txt", "a+");        
     printf("creating INTMUL file\n");
     si_data_profile_INTMUL ++;  // the reason using ++ is to prevent keep executing this if block
  }
  if(si_data_profile_INTMUL >0)
  {
     fprintf(infile_INTMUL, "%d\t%d\n", s0.as_int, s1.as_int); 
  }
	...
	result.as_int = s0.as_int * s1.as_int;
        ...
        /* Xun Jiao 01/18/2017 swap INT_MUL  note here we treat INT_MUL the same way with INT_ADD */
        if (si_fault_injection_INT_MUL > 0)
             result.as_int = swapBits_INT_MUL(result.as_int); 
	...
	struct si_inst_t *inst)
{
	...
	union si_reg_t srt;
        ...
        int bit_SQRT = 23; 
        int bit[bit_SQRT];
        int err_cnt[bit_SQRT];
        float accu[bit_SQRT]; 
        float score[bit_SQRT];

//xun jiao 11/08: profiling data for Sqrt if the si_data_profile== 1 
        if(si_data_profile_FLTSQRT == 1)
        {
             infile_SQRT = fopen("input_SQRT.txt", "a+");        
             si_data_profile_FLTSQRT ++; 
        }     
	...
	struct si_inst_t *inst)
{
	...
	if(si_data_profile_FLTADD == 1)
	{
        ...
        //Yupei 11/08/17: use memory table      
        /* 
	if(USE_ADD_TABLE)
        {
	   int layer = 0;
	...
	if(si_fault_injection_FP_ADD > 0)
	    sum.as_float = swapBits_float(sum.as_float, 0, 0.5);
	...
	struct si_inst_t *inst)
{
	...
	/* Lotad operands from registers or as a literal constant. */
	if (INST.src0 == 0xFF)
		s0.as_uint = INST.lit_cnst;
	else
		s0.as_uint = si_isa_read_reg(work_item, INST.src0);
...
/* XUN JIAO 08/16/16 dump input operands for INT MUL */
  if(si_data_profile_INTMUL == 1) {
     infile_INTMUL = fopen("input_INTMUL.txt", "a+");        
     si_data_profile_INTMUL ++;
  }
  if(si_data_profile_INTMUL >0)
  {
     fprintf(infile_INTMUL, "%d\t%d\n", s0.as_int, s1.as_int); 
	...
	product.as_int = s0.as_int * s1.as_int;
        ...
        /* Xun Jiao 01/22/2017 swap bits*/
        if (si_fault_injection_INT_MUL > 0)
             product.as_int = swapBits_INT_MUL(product.as_int);
	...
	struct si_inst_t *inst)
{
	...
	if(si_data_profile_FLTMAC == 1)
	{
        ...
        union si_reg_t tmp_mul;    // creat a temp variable for MUL result 

        // Yupei 11/08/17 Use memory table for MUL 
        /*
	if(USE_MAC_MUL_TABLE)
        ...
        */
        ...
        // Yupei 11/08/17 Use memory table for ADD 
        /*
	...
	struct si_inst_t *inst)
{
  ...
  /* xun jiao: 08/16/16, dump the input operand */
  if(si_data_profile_INTADD == 1) {
     infile_INTADD = fopen("input_INTADD.txt", "a+");        
     si_data_profile_INTADD ++; 
  }
...
//xun jiao 08/16/16: profile FLT_MUL. 
  ...
  if(si_fault_injection_FP_MUL > 0)
      result.as_float = swapBits_FP_MUL(result.as_float);
	...
	struct si_inst_t *inst)
{
  ...
  if(si_data_profile_INTMUL >0)
  {
    ...
    /* Xun Jiao 01/22/2017 swap INT_MUL bits*/
    if(si_fault_injection_INT_MUL > 0)
         product.as_int = swapBits_INT_MUL(product.as_int);
